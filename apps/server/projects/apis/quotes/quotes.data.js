export default {
  anonymous: [
    {
      id: 1,
      text: 'I don\'t have friends. I got family',
    },
    {
      id: 2,
      text: 'Ogres are like onions',
    },
    {
      id: 3,
      text: 'You\'re my boy, Blue!',
    },
    {
      id: 4,
      text: 'Jessica, only child, Illinois, Chicago',
    },
    {
      id: 5,
      text: 'You have bewitched me, body and soul',
    },
    {
      id: 6,
      text: 'So be prepared, be enthusiastic,&nbsp;and leave your bullshit attitude and baggage at the door because we don\'t need it!',
    },
    {
      id: 7,
      text: 'I just wanted to take another look at you',
    },
    {
      id: 8,
      text: 'You gotta hear this one song. It\'ll change your life, I swear',
    },
    {
      id: 9,
      text: 'This is how I win',
    },
    {
      id: 10,
      text: 'You\'ve got red on you',
    },
    {
      id: 11,
      text: 'Honest to blog?',
    },
    {
      id: 12,
      text: 'I am Groot',
    },
    {
      id: 13,
      text: 'Put some Windex on it',
    },
    {
      id: 14,
      text: 'It is the <em>titular</em> role!',
    },
    {
      id: 15,
      text: 'I want to play a game',
    },
    {
      id: 16,
      text: 'They called me Mr. Glass',
    },
    {
      id: 17,
      text: 'I have nipples, Greg. Could&nbsp;you milk me?',
    },
    {
      id: 18,
      text: 'This is Sparta!',
    },
    {
      id: 19,
      text: 'In one of our designs even these mosquito bites will look like juicy, juicy mangoes!',
    },
    {
      id: 20,
      text: 'I wanna rob',
    },
    {
      id: 21,
      text: 'You\'re putting the pussy on a pedestal',
    },
    {
      id: 22,
      text: 'Cello, you\'ve got a bass',
    },
    {
      id: 23,
      text: 'I am gonna kill Bill',
    },
    {
      id: 24,
      text: 'Didn\'t I tell you not to come to my house? Nobody touches my child!',
    },
    {
      id: 25,
      text: 'We could not talk or talk forever and still find things to not talk about',
    },
    {
      id: 26,
      text: 'Why\'d y\'spill yer beans?',
    },
    {
      id: 27,
      text: 'Hell is a teenage girl',
    },
    {
      id: 28,
      text: 'You gonna eat your tots?',
    },
    {
      id: 29,
      text: 'I have had it with these motherfuckin\' snakes on this motherfuckin\' plane!',
    },
    {
      id: 30,
      text: 'To me, you are perfect',
    },
    {
      id: 31,
      text: 'For a guy with a four digit IQ, I must have missed something',
    },
    {
      id: 32,
      text: 'My tastes are very singular',
    },
    {
      id: 33,
      text: 'I have to return some videotapes',
    },
    {
      id: 34,
      text: 'Do you know what happens to a toad when it\'s struck by lightning? The same thing that happens to everything else',
    },
    {
      id: 35,
      text: 'In moonlight, black boys look blue',
    },
    {
      id: 36,
      text: 'These are not spirit fingers. <u>These</u>&nbsp;are spirit fingers. And these are gold',
    },
    {
      id: 37,
      text: 'The law says that you cannot touch.&nbsp;But I think I see a lot of lawbreakers up in this house tonight',
    },
    {
      id: 38,
      text: 'You\'re a wizard, &#8216;arry',
    },
    {
      id: 39,
      text: 'I\'m the guy who does his job. You must be the other guy',
    },
    {
      id: 40,
      text: 'Look at my shit',
    },
    {
      id: 41,
      text: 'Prepare to be fucked by the long dick of the law!',
    },
    {
      id: 42,
      text: 'Just keep swimming',
    },
    {
      id: 43,
      text: 'You had my curiosity. But now you have my attention',
    },
    {
      id: 44,
      text: 'You\'re the man now, dog!',
    },
    {
      id: 45,
      text: 'Girl, you can\'t get no infection in your booty hole! It\'s a booty hole!',
    },
    {
      id: 46,
      text: 'I volunteer as tribute',
    },
    {
      id: 47,
      text: 'Even artichokes have hearts',
    },
    {
      id: 48,
      text: 'Would that it were so simple',
    },
    {
      id: 49,
      text: 'With great power comes great responsibility',
    },
    {
      id: 50,
      text: 'Ass to ass',
    },
    {
      id: 51,
      text: 'You will ride eternal, shiny and chrome',
    },
    {
      id: 52,
      text: 'I don\'t like sand. It\'s all coarse, and rough, and irritating. And it gets everywhere',
    },
    {
      id: 53,
      text: 'I am the motherfucker that found this place, sir',
    },
    {
      id: 54,
      text: 'Kiss me, my girl, before I am sick',
    },
    {
      id: 55,
      text: 'The wrong kid died',
    },
    {
      id: 56,
      text: 'Not the bees!',
    },
    {
      id: 57,
      text: 'I\'m glad he\'s single because I\'m going to climb that like a tree',
    },
    {
      id: 58,
      text: 'Are you watching closely?',
    },
    {
      id: 59,
      text: 'Dear 8-pound, 6-ounce newborn infant Jesus&#8230;',
    },
    {
      id: 60,
      text: 'Man, fuck Jesse Jackson!',
    },
    {
      id: 61,
      text: 'Would it be all right if I showed the children the whoring bed?',
    },
    {
      id: 62,
      text: 'I was perfect',
    },
    {
      id: 63,
      text: 'That\'s a bingo',
    },
    {
      id: 64,
      text: 'Why are you wearing that stupid man suit?',
    },
    {
      id: 65,
      text: 'I know that babies taste the best',
    },
    {
      id: 66,
      text: 'Honey? Where\'s my super suit?',
    },
    {
      id: 67,
      text: 'What is this? A center for ants?',
    },
    {
      id: 68,
      text: 'Meet me in Montauk',
    },
    {
      id: 69,
      text: 'It\'s the fucking Catalina Wine Mixer',
    },
    {
      id: 70,
      text: 'Rock stars have kidnapped my son!',
    },
    {
      id: 71,
      text: 'Not quite my tempo',
    },
    {
      id: 72,
      text: 'Wilsoooooooon!',
    },
    {
      id: 73,
      text: 'Now you\'re in the sunken place',
    },
    {
      id: 74,
      text: 'Why so serious?',
    },
    {
      id: 75,
      text: 'Difficult difficult lemon difficult',
    },
    {
      id: 76,
      text: 'I won\'t say it for Little Baby Ears over there, but it rhymes with &#8216;smushmortion.\'',
    },
    {
      id: 77,
      text: 'Is this your king?',
    },
    {
      id: 78,
      text: 'If you\'re a bird, I\'m a bird',
    },
    {
      id: 79,
      text: 'I am Shiva, the god of death',
    },
    {
      id: 80,
      text: 'I\'m gonna steal the Declaration of Independence',
    },
    {
      id: 81,
      text: 'You shall not pass!',
    },
    {
      id: 82,
      text: 'If anyone orders merlot, I\'m leaving. I am <u><em>not</em></u> drinking any fucking merlot!',
    },
    {
      id: 83,
      text: 'But what I do have are a very particular set of skills, skills I have acquired over a very long career, skills that make me a nightmare for people like you',
    },
    {
      id: 84,
      text: 'Look at me. Look at me. I am the captain now',
    },
    {
      id: 85,
      text: 'Baby, you are gonna miss that plane',
    },
    {
      id: 86,
      text: 'My wife!',
    },
    {
      id: 87,
      text: 'Are you not entertained?',
    },
    {
      id: 88,
      text: 'You sit on a throne of lies',
    },
    {
      id: 89,
      text: 'Wouldst thou like to live deliciously?',
    },
    {
      id: 90,
      text: 'Exercise gives you endorphins. Endorphins make you happy. Happy people just don\'t shoot their husbands, they just don\'t',
    },
    {
      id: 91,
      text: 'You\'re tearing me apart, Lisa!',
    },
    {
      id: 92,
      text: 'I\'m a fiend for mojitos',
    },
    {
      id: 93,
      text: 'Boy, that escalated quickly',
    },
    {
      id: 94,
      text: 'King Kong ain\'t got shit on me!',
    },
    {
      id: 95,
      text: 'I live my life a quarter mile at a time',
    },
    {
      id: 96,
      text: 'A million dollars isn\'t cool. You know what\'s cool?',
    },
    {
      id: 97,
      text: 'She doesn\'t even go here!',
    },
    {
      id: 98,
      text: 'I wish I knew how to quit you',
    },
    {
      id: 99,
      text: 'Florals? For spring? Groundbreaking',
    },
    {
      id: 100,
      text: 'I drink your milkshake',
    },
  ],
  authored: [
    {
      id: 101,
      text: 'Frankly, my dear, I don\'t give a damn.',
      author: 'Gone with the Wind',
    },
    {
      id: 102,
      text: 'I\'m gonna make him an offer he can\'t refuse.',
      author: 'The Godfather',
    },
    {
      id: 103,
      text: 'You don\'t understand! I coulda had class. I coulda been a contender. I could\'ve been somebody, instead of a bum, which is what I am.',
      author: 'On the Waterfront',
    },
    {
      id: 104,
      text: 'Toto, I\'ve a feeling we\'re not in Kansas anymore.',
      author: 'The Wizard of Oz',
    },
    {
      id: 105,
      text: 'Here\'s looking at you, kid.',
      author: 'Casablanca',
    },
    {
      id: 106,
      text: 'Go ahead, make my day.',
      author: 'Sudden Impact',
    },
    {
      id: 107,
      text: 'All right, Mr. DeMille, I\'m ready for my close-up.',
      author: 'Sunset Boulevard',
    },
    {
      id: 108,
      text: 'May the Force be with you.',
      author: 'Star Wars',
    },
    {
      id: 109,
      text: 'Fasten your seatbelts. It\'s going to be a bumpy night.',
      author: 'All About Eve',
    },
    {
      id: 110,
      text: 'You talkin\' to me?',
      author: 'Taxi Driver',
    },
    {
      id: 111,
      text: 'What we\'ve got here is failure to communicate.',
      author: 'Cool Hand Luke',
    },
    {
      id: 112,
      text: 'I love the smell of napalm in the morning.',
      author: 'Apocalypse Now',
    },
    {
      id: 113,
      text: 'Love means never having to say you\'re sorry.',
      author: 'Love Story',
    },
    {
      id: 114,
      text: 'The stuff that dreams are made of.',
      author: 'The Maltese Falcon',
    },
    {
      id: 115,
      text: 'E.T. phone home.',
      author: 'E.T. the Extra-Terrestrial',
    },
    {
      id: 116,
      text: 'They call me Mister Tibbs!',
      author: 'In the Heat of the Night',
    },
    {
      id: 117,
      text: 'Rosebud.',
      author: 'Citizen Kane',
    },
    {
      id: 118,
      text: 'Made it, Ma! Top of the world!',
      author: 'White Heat',
    },
    {
      id: 119,
      text: 'I\'m as mad as hell, and I\'m not going to take this anymore!',
      author: 'Network',
    },
    {
      id: 120,
      text: 'Louis, I think this is the beginning of a beautiful friendship.',
      author: 'Casablanca',
    },
    {
      id: 121,
      text: 'A census taker once tried to test me. I ate his liver with some fava beans and a nice Chianti.',
      author: 'The Silence of the Lambs',
    },
    {
      id: 122,
      text: 'Bond. James Bond.',
      author: 'Dr. No',
    },
    {
      id: 123,
      text: 'There\'s no place like home.',
      author: 'The Wizard of Oz',
    },
    {
      id: 124,
      text: 'I am big! It\'s the pictures that got small.',
      author: 'Sunset Boulevard',
    },
    {
      id: 125,
      text: '"Show me the money!',
      author: 'Jerry Maguire',
    },
    {
      id: 126,
      text: 'Why don\'t you come up sometime and see me?',
      author: 'She Done Him Wrong',
    },
    {
      id: 127,
      text: 'I\'m walking here! I\'m walking here!',
      author: 'Midnight Cowboy',
    },
    {
      id: 128,
      text: 'Play it, Sam. Play \'As Time Goes By.\'',
      author: 'Casablanca',
    },
    {
      id: 129,
      text: 'You can\'t handle the tr!',
      author: 'A Few Good Men',
    },
    {
      id: 130,
      text: 'I want to be alone.',
      author: 'Grand Hotel',
    },
    {
      id: 131,
      text: '"After all, tomorrow is another day!',
      author: 'Gone with the Wind',
    },
    {
      id: 132,
      text: 'Round up the usual suspects.',
      author: 'Casablanca',
    },
    {
      id: 133,
      text: 'I\'ll have what she\'s having.',
      author: 'When Harry Met Sally...',
    },
    {
      id: 134,
      text: 'You know how to whistle, don\'t you, Steve? You just put your lips together and blow.',
      author: 'To Have and Have Not',
    },
    {
      id: 135,
      text: 'You\'re gonna need a bigger boat.',
      author: 'Jaws',
    },
    {
      id: 136,
      text: 'Badges? We ain\'t got no badges! We don\'t need no badges! I don\'t have to show you any stinking badges!',
      author: 'Gold Hat',
    },
    {
      id: 137,
      text: 'I\'ll be back.',
      author: 'The Terminator',
    },
    {
      id: 138,
      text: 'Today, I consider myself the luckiest man on the face of the Earth.',
      author: 'The Pride of the Yankees',
    },
    {
      id: 139,
      text: 'If you build it, he will come.',
      author: 'Field of Dreams',
    },
    {
      id: 140,
      text: 'Mama always said life was like a box of chocolates. You never know what you\'re gonna get.',
      author: 'Forrest Gump',
    },
    {
      id: 141,
      text: 'We rob banks.',
      author: 'Bonnie and Clyde',
    },
    {
      id: 142,
      text: 'Plastics.',
      author: 'The Graduate',
    },
    {
      id: 143,
      text: 'We\'ll always have Paris.',
      author: 'Casablanca',
    },
    {
      id: 144,
      text: 'I see dead people.',
      author: 'The Sixth Sense',
    },
    {
      id: 145,
      text: '"Stella! Hey, Ste',
      author: '!" A Streetcar Named Desire',
    },
    {
      id: 146,
      text: 'Oh, Jerry, don\'t let\'s ask for the moon. We have the stars.',
      author: 'Now, Voyager',
    },
    {
      id: 147,
      text: '"Shane. Shane. Come back!',
      author: 'Shane',
    },
    {
      id: 148,
      text: 'Well, nobody\'s perfect.',
      author: 'Some Like It Hot',
    },
    {
      id: 149,
      text: 'It\'s alive! It\'s alive!',
      author: 'Frankenstein',
    },
    {
      id: 150,
      text: 'Houston, we have a problem.',
      author: 'Apollo 13',
    },
    {
      id: 151,
      text: 'You\'ve got to ask yourself one question: \'Do I feel lucky?\' Well, do ya, punk?',
      author: 'Dirty Harry',
    },
    {
      id: 152,
      text: 'You had me at \'hello.\'',
      author: 'Jerry Maguire',
    },
    {
      id: 153,
      text: 'One morning I shot an elephant in my pajamas. How he got in my pajamas, I don\'t know.',
      author: 'Animal Crackers',
    },
    {
      id: 154,
      text: 'There\'s no crying in baseball!',
      author: 'A League of Their Own',
    },
    {
      id: 155,
      text: 'La-dee-da, la-dee-da.',
      author: 'Annie Hall',
    },
    {
      id: 156,
      text: 'A boy\'s best friend is his mother.',
      author: 'Psycho',
    },
    {
      id: 157,
      text: 'Greed, for lack of a better word, is good.',
      author: 'Wall Street',
    },
    {
      id: 158,
      text: 'Keep your friends close, but your enemies closer.',
      author: 'The Godfather Part II',
    },
    {
      id: 159,
      text: 'As God is my witness, I\'ll never be hungry again.',
      author: 'Gone with the Wind',
    },
    {
      id: 160,
      text: 'Well, here\'s another nice mess you\'ve gotten me into!',
      author: 'Sons of the Desert',
    },
    {
      id: 161,
      text: 'Say \'hello\' to my little friend!',
      author: 'Scarface',
    },
    {
      id: 162,
      text: 'What a dump.',
      author: 'Beyond the Forest',
    },
    {
      id: 163,
      text: 'Mrs. Robinson, you\'re trying to seduce me. Aren\'t you?',
      author: 'The Graduate',
    },
    {
      id: 164,
      text: 'Gentlemen, you can\'t fight in here! This is the War Room!',
      author: 'Dr. Strangelove or: How I Learned to Stop Worrying and Love the Bomb',
    },
    {
      id: 165,
      text: 'Elementary, my dear Watson.',
      author: 'The Adventures of Sherlock Holmes',
    },
    {
      id: 166,
      text: 'Get your stinking paws off me, you damned dirty ape.',
      author: 'Planet of the Apes',
    },
    {
      id: 167,
      text: 'Of all the gin joints in all the towns in all the world, she walks into mine.',
      author: 'Casablanca',
    },
    {
      id: 168,
      text: 'Here\'s Johnny!',
      author: 'The Shining',
    },
    {
      id: 169,
      text: 'They\'re here!',
      author: 'Poltergeist',
    },
    {
      id: 170,
      text: 'Is it safe?" Marathon Man',
      author: '',
    },
    {
      id: 171,
      text: 'Wait a minute, wait a minute. You ain\'t heard nothin\' yet!',
      author: 'The Jazz Singer',
    },
    {
      id: 172,
      text: 'No wire hangers, ever!',
      author: 'Mommie Dearest',
    },
    {
      id: 173,
      text: 'Mother of mercy, is this the end of Rico?',
      author: 'Little Caesar',
    },
    {
      id: 174,
      text: 'Forget it, Jake, it\'s Chinatown.',
      author: 'Chinatown',
    },
    {
      id: 175,
      text: 'I have always depended on the kindness of strangers.',
      author: 'A Streetcar Named Desire',
    },
    {
      id: 176,
      text: 'Hasta la vista, baby.',
      author: 'Terminator 2: Judgment Day',
    },
    {
      id: 177,
      text: 'Soylent Green is people!',
      author: 'Soylent Green',
    },
    {
      id: 178,
      text: 'Open the pod bay doors, HAL.',
      author: '2001: A Space Odyssey',
    },
    {
      id: 179,
      text: 'Yo, Adrian!',
      author: 'Rocky',
    },
    {
      id: 180,
      text: 'Hello, gorgeous.',
      author: 'Funny Girl',
    },
    {
      id: 181,
      text: 'Toga! Toga!',
      author: 'National Lampoon\'s Animal House',
    },
    {
      id: 182,
      text: 'Listen to them. Children of the night. What music they make.',
      author: 'Dracula',
    },
    {
      id: 183,
      text: 'Oh, no, it wasn\'t the airplanes. It was Beauty killed the Beast.',
      author: 'King Kong',
    },
    {
      id: 184,
      text: 'My precious.',
      author: 'The Lord of the Rings: The Two Towers',
    },
    {
      id: 185,
      text: 'Attica! Attica!',
      author: 'Dog Day Afternoon',
    },
    {
      id: 186,
      text: 'Sawyer, you\'re going out a youngster, but you\'ve got to come back a star!',
      author: '42nd Street',
    },
    {
      id: 187,
      text: 'Listen to me, mister. You\'re my knight in shining armor. Don\'t you forget it. You\'re going to get back on that horse, and I\'m going to be right behind you, holding on tight, and away we\'re gonna go, go, go!',
      author: 'On Golden Pond',
    },
    {
      id: 188,
      text: 'Tell \'em to go out there with all they got and win just one for the Gipper.',
      author: 'Knute Rockne, All American',
    },
    {
      id: 189,
      text: 'A martini. Shaken, not stirred.',
      author: 'Goldfinger',
    },
    {
      id: 190,
      text: 'Who\'s on first?',
      author: 'The Naughty Nineties',
    },
    {
      id: 191,
      text: 'Cinderella story. Outta nowhere. A former greenskeeper, now, about to become the Masters champion. It looks like a mirac...It\'s in the hole! It\'s in the hole! It\'s in the hole!',
      author: 'Caddyshack',
    },
    {
      id: 192,
      text: 'Life is a banquet, and most poor suckers are starving to death!',
      author: 'Auntie Mame',
    },
    {
      id: 193,
      text: 'I feel the need—the need for speed!',
      author: 'Top Gun',
    },
    {
      id: 194,
      text: 'Carpe diem. Seize the day, boys. Make your lives extraordinary.',
      author: 'Dead Poets Society',
    },
    {
      id: 195,
      text: 'Snap out of it!',
      author: 'Moonstruck',
    },
    {
      id: 196,
      text: 'My mother thanks you. My father thanks you. My sister thanks you. And I thank you.',
      author: 'Yankee Doodle Dandy',
    },
    {
      id: 197,
      text: 'Nobody puts Baby in a corner.',
      author: 'Dirty Dancing',
    },
    {
      id: 198,
      text: 'I\'ll get you, my pretty, and your little dog too!',
      author: 'The Wizard of Oz',
    },
    {
      id: 199,
      text: 'I\'m the King of the World.',
      author: 'Titanic',
    },
  ],
};
